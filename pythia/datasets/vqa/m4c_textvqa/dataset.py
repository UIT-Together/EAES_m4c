...
class M4CTextVQADataset(TextVQADataset):
    def __init__(self, dataset_type, imdb_file_index, config, *args, **kwargs):
        ...
        # self.VG_sg_class_path = './VG_sg_features/VG_sg_classes/'
        # self.textcaps_train_test_split = json.load(open('./VG_sg_features/train_test_split.json'))
    ...
    def add_sample_details(self, sample_info, sample):
        ...
        # sample.rela_matrix = np.load(self.VG_sg_feature_path + '/' + sample_info["image_id"] + '.npy')
        # sample.classes = np.load(self.VG_sg_class_path + '/' + self.textcaps_train_test_split[sample_info["image_id"]] + '/' + sample_info["image_id"] + '_classes.npy')
