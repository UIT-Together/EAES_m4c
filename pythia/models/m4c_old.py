...
class M4C(BaseModel):
    ...
    def _build_txt_encoding(self):
        ...
        if self.config.text_bert_init_from_bert_base:
            ...
            self.text_bert = VisualBertModel.from_pretrained("uclanlp/visualbert-vqa-coco-pre")
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        fwd_results['obj_ori_feat'] = obj_feat_ori
        fwd_results['obj_ori_grid'] = obj_grid
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        fwd_results['ocr_ori_feat'] = ocr_fc7
    ...
    def _forward_mmt(self, sample_list, fwd_results):
        ...
        text_bert_out = self.text_bert( 
            input_ids=fwd_results['txt_inds'], 
            attention_mask=fwd_results['txt_mask'], 
            visual_embeds=fwd_results['ocr_ori_feat'], 
            visual_attention_mask=fwd_results['ocr_mask']).last_hidden_state
        
        text_bert_out = text_bert_out.mean(dim=1).view(text_bert_out.shape[0], 1, text_bert_out.shape[-1])
...
class MMT(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        encoder_inputs = torch.cat(
            [txt_emb, obj_emb, ocr_emb, dec_emb], # 151 + 100 + 50 + 30
            dim=1
        )
        attention_mask = torch.cat(
            [txt_mask, obj_mask, ocr_mask, dec_mask], # 1 + 100 + 50 + 30
            dim=1
        )
...
@registry.register_model("m4c_1024d")
class M4C_1024d(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.mmt_config = BertConfig(**self.config.mmt)
        self._datasets = registry.get("config").datasets.split(",")
