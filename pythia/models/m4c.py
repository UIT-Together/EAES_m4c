...
class M4C(BaseModel):
    def __init__(self, config):
        ...
        self.ft_embbed = np.load('./VG_sg_features/cls_sg_fasttext.npy', allow_pickle=True).tolist()
        self.ft_embbed_arr = np.array([self.ft_embbed[i] for i in self.ft_embbed])
        self.rela_cls = json.load(open('./VG-SGG-dicts-danfeiX-clipped.json'))['idx_to_predicate']
    ...
    def _build_obj_encoding(self):
        ...
        # objs relative linear 8/11/2021
        ...
        self.linear_grid_feat_to_mmt_in = nn.Linear(
            2048, self.mmt_config.hidden_size
        )
        ...
        # Doanh BC 9/1/2022 scene graph feature layer norm
        # self.sbj_obj_layer_norm = BertLayerNorm(self.mmt_config.hidden_size)
        # self.obj_sbj_layer_norm = BertLayerNorm(self.mmt_config.hidden_size)
        self.scene_graph_feature_layer_norm = BertLayerNorm(self.mmt_config.hidden_size)

        # DoanhBC 9/11/2021 grid layer norm
        self.obj_grid_layer_norm = BertLayerNorm(self.mmt_config.hidden_size)
        
        # DoanhBC 8/11/2021 Relative layer norm
        ...
        # weight geometry for objs bbox 8/11/2021
        self.obj_WGs = nn.Linear(64, 1, bias=True)

        # Spatial Graph Convolution add 9/1/2022
        # in_dim = 2348 = 1024 (sbj/obj R50 feature) + 1024 (sbj/obj R50 feature) + 300 (class scene)
        self.ssg_sbj_rela_fc = nn.Sequential(nn.Linear(2348, self.mmt_config.hidden_size),
                                         nn.ReLU(inplace=True),
                                         nn.Dropout(self.config.obj.dropout_prob))

        self.ssg_obj_rela_fc = nn.Sequential(nn.Linear(2348, self.mmt_config.hidden_size),
                                         nn.ReLU(inplace=True),
                                         nn.Dropout(self.config.obj.dropout_prob))
    ...
    def _build_ocr_encoding(self):
        ...
        # weight geometry for ocr tokens 8/11/2021
        self.ocr_WGs = nn.Linear(64, 1, bias=True)
        
        # objs relative linear 10/11/2021
        self.linear_ocr_rg_to_mmt_in = nn.Linear(
            50, self.mmt_config.hidden_size
        )
        
        # DoanhBC 10/11/2021 OCR Relative layer norm
        self.ocr_relative_layer_norm = BertLayerNorm(self.mmt_config.hidden_size)
        self.ocr_drop = nn.Dropout(self.config.ocr.dropout_prob)

        # Doanh BC 11/1/2022 linear projection for OCR relative feature
        # self.linear_rela_ocr_feat_to_ptrocrnet_in = nn.Linear(
        #     self.mmt_config.hidden_size + self.config.classifier.ocr_max_num, self.mmt_config.hidden_size
        # )

        # DoanhBC 13/1/2022 add r50 feature
        # self.r50_feature_upsampling = nn.Linear(1024, 2048)
        # self.linear_r50_to_mmt_in = nn.Linear(2048, self.mmt_config.hidden_size)
        # self.r50_layer_norm = BertLayerNorm(self.mmt_config.hidden_size)
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        obj_feat_ori = obj_fc6
        # obj_feat_r50 = sample_list.r50_feature
        # obj_feat_r50 = torch.from_numpy(np.array(obj_feat_r50)).to(obj_fc6.device)
        
        obj_grid = sample_list.grid_features
        obj_bbox = sample_list.obj_bbox_coordinates

        # DoanhBC add 10/11/2021 relative geometry for OCR tokens.
        ...
        relative_geometry_weights = self.ocr_WGs(flatten_relative_geometry_embeddings.float()).view(box_size_per_head)
        objs_relative_geometry = relative_geometry_weights.view(relative_geometry_weights.shape[0], relative_geometry_weights.shape[-2], relative_geometry_weights.shape[-1])

        # rela_matrix = sample_list.rela_matrix # (bs, N_rela, 3)

        # # Scene graph feature idea 2

        # sbj_obj_scene_graph = torch.zeros(len(rela_matrix), 100, 2348).to(obj_bbox.device) # scene feature (bs, 100, 2048)
        # obj_sbj_scene_graph = torch.zeros(len(rela_matrix), 100, 2348).to(obj_bbox.device) # scene feature (bs, 100, 2048)

        # for batch_idx in range(len(rela_matrix)):
        #     N_rela = len(rela_matrix[batch_idx])
        #     feat_ori = obj_feat_ori[batch_idx]
        #     subj_ids = rela_matrix[batch_idx][:,0]
        #     obj_ids = rela_matrix[batch_idx][:,1]
        #     rela_ids = list(map(int, rela_matrix[batch_idx][:,2]))
        #     rela_ids = list(np.array(rela_ids) - np.array([1]*len(rela_ids)))
        #     predicates = torch.from_numpy(self.ft_embbed_arr[rela_ids]).to(obj_bbox.device)
        #     subjs_feat = feat_ori[subj_ids]
        #     objs_feat = feat_ori[obj_ids]
        #     sbj_obj_scene_graph[batch_idx][:N_rela,:] = torch.cat((subjs_feat, objs_feat, predicates), dim=1)
        #     obj_sbj_scene_graph[batch_idx][:N_rela,:] = torch.cat((objs_feat, subjs_feat, predicates), dim=1)

        # import pdb; pdb.set_trace()
        # DoanhBC 9/11 add grid features
        obj_mmt_in = (
            self.obj_feat_layer_norm(self.linear_obj_feat_to_mmt_in(obj_feat_ori)) + # box feature
            self.obj_bbox_layer_norm(self.linear_obj_bbox_to_mmt_in(obj_bbox.type(torch.float32))) + # bounding box 
            self.obj_grid_layer_norm(self.linear_grid_feat_to_mmt_in(obj_grid))
            # self.obj_relative_layer_norm(self.linear_obj_rg_to_mmt_in(objs_relative_geometry.type(torch.float32))) + # relative
            # self.sbj_obj_layer_norm(self.ssg_sbj_rela_fc(sbj_obj_scene_graph)) + # sbj_obj_rela features
            # self.obj_sbj_layer_norm(self.ssg_obj_rela_fc(obj_sbj_scene_graph)) # obj_sbj_rela features
        )
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        relative_geometry_embeddings = BoxRelationalEmbedding(ocr_bbox)
        flatten_relative_geometry_embeddings = relative_geometry_embeddings.view(-1, 64)
        box_size_per_head = list(relative_geometry_embeddings.shape[:3])
        box_size_per_head.insert(1, 1)
        relative_geometry_weights = self.ocr_WGs(flatten_relative_geometry_embeddings).view(box_size_per_head)
        # BCD fix 7/1/2022
        ocr_relative_geometry = relative_geometry_weights.view(relative_geometry_weights.shape[0], relative_geometry_weights.shape[-2], relative_geometry_weights.shape[-1])
        ...
        ocr_mmt_in = (
            ...
            ) + self.ocr_relative_layer_norm(
                self.linear_ocr_rg_to_mmt_in(ocr_relative_geometry)
            )
        )
        ...
        # BCD add 7/1/2022
        fwd_results['ocr_mmt_rg_in'] = ocr_relative_geometry
    ...
    def _forward_mmt(self, sample_list, fwd_results):
        ...
        mmt_results = self.mmt(
            ...
            # Add 7/1/2022
            # obj_rg=fwd_results['obj_mmt_rg_in'],
            # ocr_rg=fwd_results['ocr_mmt_rg_in'],
        )
