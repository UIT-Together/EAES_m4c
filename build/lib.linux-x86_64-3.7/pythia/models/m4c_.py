...
class M4C(BaseModel):
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        relative_geometry_embeddings = self.BoxRelationalEmbedding(obj_bbox.unsqueeze(0))
        flatten_relative_geometry_embeddings = relative_geometry_embeddings.view(-1, 64)
        box_size_per_head = list(relative_geometry_embeddings.shape[:3])
        box_size_per_head.insert(1, 1)
        relative_geometry_weights = WGs(flatten_relative_geometry_embeddings).view(box_size_per_head)
        objs_relative_geometry = relative_geometry_weights[0][0]

        obj_mmt_in = (
            self.obj_feat_layer_norm(self.linear_obj_feat_to_mmt_in(obj_feat)) + 
            self.obj_bbox_layer_norm(self.linear_obj_bbox_to_mmt_in(obj_bbox)) +
            self.obj_relative_layer_norm(self.linear_obj_relative_geometry_to_mmt_in(objs_relative_geometry))
        )
